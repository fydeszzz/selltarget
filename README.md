...
* 從底部導覽列切換分頁：**手續費折數** 反推你的券商折數，**設定** 包含語言、關於與問題回報資訊
...
2. **手續費倍率**：台股有手續費倍率，可搭配 **手續費折數** 分頁計算。例如 4 折請輸入 `0.4`，留空 = 原價（成交金額較小時倍率不影響）
...
★ 如何使用 Sell Signal：盤中看到機會，他點選【賣點試算】，用報酬率模式快速抓出短線目標賣價；再切到【手續費折數】，用券商折數反推這筆交易的實際手續費，連同證交稅一併試算，秒算出這筆淨利是否值得做。每一筆都先算清楚再下單，幫助他遵守紀律。
